...
// ✅ Removed Safety category
// ✅ Added Nutrition category
// ✅ Nutrition resources now map to Nutrition (instead of defaulting to SRH)
// ✅ Former Safety topics (consent/safety) now map into SRH so they don’t disappear
...
const TOPIC_TO_CATEGORY = {
  ...
  sti_hiv: "SRH",
  pregnancy: "SRH",
  emergency_contraception: "SRH",

  // Previously Safety → now SRH (since Safety category is removed)
  consent: "SRH",
  safety: "SRH",
...
function safeParseLesson(content) {
...
const i18n = {
  sw: {
    pageTitle: "Jifunze",
    pageSubtitle: "Majibu ya haraka unayoweza kuyaamini — salama, rahisi na rafiki kwa vijana.",
    ...
    searchPh: "Tafuta rasilimali…",
    filterAll: "Zote",
    filterSRH: "SRH",
    filterNutrition: "Lishe",
    filterGBV: "GBV",
    filterMental: "Afya ya akili",
    filterByLang: "Chuja kwa Lugha",
    filterByType: "Chuja kwa Aina",
    allLanguages: "Lugha Zote",
    allTypes: "Aina Zote",
    loading: "Inapakia…",
    empty: "Hakuna rasilimali.",
    ...
    steps: "Hatua za Kujitunza",
    mythFact: "Hadithi vs Ukweli",
    quiz: "Jaribio Fupi",
    answers: "Majibu",
    dangerSigns: "Dalili Hatarishi",
    support: "Msaada",
  },
  en: {
    pageTitle: "Learn",
    pageSubtitle: "Quick guidance you can trust — safe, clear, youth-friendly.",
    backHome: "Back Home",
...
export default function Resources() {
    ...
    (async () => {
      ...
      try {
        const { data, error } = await supabase
          .from("resources")
          .select(
            "id, title, topic, language, content, resource_type, video_url, thumbnail_url, tags, published, updated_at"
          )
          .eq("published", true)
  ...
  const resources = useMemo(() => {
    ...
    for (const [topic, bucket] of byTopic.entries()) {
      ...
      const lesson = safeParseLesson(chosen.content);
      const plainText =
        typeof chosen.content === "string" ? chosen.content : JSON.stringify(chosen.content ?? "");

      const displayTitle = (lesson && lesson.title) || chosen.title || topic;

      const summaryText = lesson
        ? (() => {
            const narrator = (lesson.content_blocks || []).find((b) => b.type === "narrator");
            const text = narrator?.text || "";
            const s = (text || "").replace(/\s+/g, " ").trim();
            if (!s) return "";
            return s.length <= 150 ? s : s.slice(0, 149).trim() + "…";
          })()
        : makeSummary(plainText || "", 150);

      const category = TOPIC_TO_CATEGORY[topic] || "SRH";
      ...
      list.push({
        ...
        title: displayTitle,
        summary: summaryText,
        body: lesson ? [] : splitParagraphs(plainText || ""),
        parsed: lesson || null,
    ...
    // ✅ Updated order: No Safety, Nutrition inserted
    const order = { SRH: 1, Nutrition: 2, GBV: 3, Mental: 4 };
    list.sort((a, b) => {
  ...
  const suggestions = useMemo(() => {
    ...
    const titleMatches = resources
      ...
      .slice(0, 3)
      .map((r) => ({ type: "title", text: r.title, id: r.id }));
    ...
    <div className="min-h-screen bg-white">
      <section className="mx-auto max-w-[1200px] px-4 py-12 sm:py-16">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-4xl tracking-tight sm:text-5xl" style={{ fontFamily: "ui-serif, Georgia, Cambria, serif" }}>
              {t.pageTitle}
            </h1>
            ...
            <div className="absolute z-10 mt-2 w-full rounded-2xl border border-slate-200 bg-white shadow-lg">
              {suggestions.map((s, idx) => (
                <button
                  key={idx}
                  onClick={() => {
                    if (s.type === "title") setOpenId(s.id);
                    else {
                      setSelectedTag(s.text);
                      setQuery("");
                    }
                ...
                >
                  <span className="mr-2">{s.type === "title" ? "📄" : "🏷️"}</span>
                  {s.text}
                </button>
              ))}
            </div>
        ...
        <div className="mt-6 space-y-4">
          <div className="flex flex-wrap gap-2">
            {/* ✅ Removed Safety, added Nutrition */}
            {["All", "SRH", "Nutrition", "GBV", "Mental"].map((f) => (
          ...
          <div className="flex flex-wrap gap-4">
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="rounded-lg border border-slate-200 px-3 py-1.5 text-sm outline-none focus:border-slate-300"
              >
          ...
          {selectedTag && (
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700 hover:bg-amber-200"
              >
            ...
            <div className="col-span-full rounded-3xl border border-slate-200 bg-white p-7 text-center text-sm text-slate-600">
              ...
              {(query || selectedTag) && (
                <button
                  ...
                  className="mt-3 text-amber-600 hover:text-amber-700 font-semibold"
                >
                  Clear filters
                </button>
              )}
      ...
      {openItem && (
        <ResourceModal
          ...
          onRelatedClick={setOpenId}
          closeBtnRef={modalCloseBtnRef}
          t={t}
        />
      )}

      {shareNotification && (
        <div className="fixed bottom-6 right-6 z-50 rounded-2xl bg-slate-900 px-6 py-3 text-sm font-semibold text-white shadow-lg">
          ✓ {t.linkCopied}
        </div>
      )}
    </div>
...
function ResourceCard({ resource, onClick, onTagClick, t }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group text-left rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-[1px] hover:shadow-md"
    >
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {resource.tags.slice(0, 3).map((tag, idx) => (
            <span
              ...
              className="rounded-full bg-amber-50 px-2 py-0.5 text-[10px] font-semibold text-amber-700 hover:bg-amber-100 cursor-pointer"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="sticky top-0 bg-white border-b border-slate-200 p-6 rounded-t-3xl z-10">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              <div className="text-2xl tracking-tight text-slate-900 font-bold" style={{ fontFamily: "ui-serif, Georgia, Cambria, serif" }}>
                {resource.title}
              </div>
              <div className="mt-3 flex flex-wrap gap-2 text-xs">
                <span className="rounded-full bg-slate-100 px-3 py-1 font-semibold text-slate-700">
                  {RESOURCE_TYPE_ICONS[resource.resourceType]} {resource.resourceType}
                </span>
        ...
        <div className="p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {resource.tags.map((tag, idx) => (
                  <button
                    ...
                    className="rounded-full bg-amber-50 px-3 py-1 text-sm font-semibold text-amber-700 hover:bg-amber-100"
                  >
                    {tag}
                  </button>
                ))}
          ...
          <div className="space-y-4">
            {lesson?.content_blocks?.length ? (
              <LessonRenderer lesson={lesson} t={t} />
            ) : resource.body?.length ? (
              resource.body.map((p, idx) => (
                ...
                {relatedResources.map((r) => (
                  ...
                  >
                    <div className="text-xs font-semibold text-slate-500 mb-1">
                      {RESOURCE_TYPE_ICONS[r.resourceType]} {r.resourceType}
                    </div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-200">
            ...
            <div className="flex flex-wrap gap-2">
              <button onClick={() => onShare(resource, "twitter")} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-semibold hover:bg-slate-50">
                Twitter
              </button>
              <button onClick={() => onShare(resource, "facebook")} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-semibold hover:bg-slate-50">
                Facebook
              </button>
              <button onClick={() => onShare(resource, "whatsapp")} className="rounded-2xl border border-slate-200 px-4 py-2 text-sm font-semibold hover:bg-slate-50">
                WhatsApp
              </button>
              <button onClick={() => onShare(resource, "copy")} className="rounded-2xl bg-amber-500 px-4 py-2 text-sm font-semibold text-white hover:bg-amber-600">
                {t.copyLink}
              </button>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="button" onClick={onClose} className="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold hover:bg-slate-50">
              {t.close}
            </button>
          </div>
        </div>
...
/* =========================
   Lesson Renderer (minimal)
========================= */

function LessonRenderer({ lesson, t }) {
  const goals = lesson.learning_goals || [];
    ...
    <div className="space-y-4">
      {goals.length > 0 && (
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="text-sm font-extrabold text-slate-900 mb-2">{t.whatYouLearn}</div>
      ...
      {blocks.map((block, idx) => (
        <LessonBlock key={idx} block={block} t={t} />
      ))}
    </div>
  );
}

function LessonBlock({ block, t }) {
  const Card = ({ title, children }) => (
    <div className="rounded-2xl border border-slate-200 bg-white p-4">
      {title ? <div className="text-sm font-extrabold text-slate-900 mb-2">{title}</div> : null}
      {children}
    </div>
  );
  ...
  switch (block.type) {
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {(block.cards || []).map((c, i) => (
              <div key={i} className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                <div className="text-sm font-extrabold text-slate-900">{c.heading}</div>
                <div className="mt-1 text-sm leading-7 text-slate-700">{c.text}</div>
              </div>
            ))}
          </div>
        </Card>
    ...
    case "self_care_steps":
      return (
        <Card title={block.title || t.steps}>
          <ul className="list-disc pl-5 space-y-1 text-sm text-slate-700">
            {(block.steps || []).map((s, i) => (
              <li key={i}>{s}</li>
            ))}
    ...
    case "myth_fact":
      return (
        <Card title={t.mythFact}>
          <div className="space-y-3">
            {(block.pairs || []).map((p, i) => (
              <div key={i} className="rounded-2xl border border-slate-200 p-3">
                <div className="text-[11px] font-bold text-slate-500">Myth</div>
                <div className="text-sm font-extrabold text-slate-900">{p.myth}</div>
                <div className="mt-2 text-[11px] font-bold text-slate-500">Fact</div>
                <div className="text-sm leading-7 text-slate-700">{p.fact}</div>
              </div>
            ))}
          </div>
        </Card>
