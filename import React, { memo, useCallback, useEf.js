...
/**
 * SWAHIBA E-commerce MVP (Front-end only)
 * Improvements:
 * - Removed unnecessary useMemo for static data
 * - Faster lookup for Today’s Drop (Set)
 * - useCallback + memoized ProductCard to reduce re-renders
 * - Replaced line-clamp-* with Tailwind-safe truncation (no plugin needed)
 * - Added basic modal/drawer accessibility: Escape-to-close + focus restore
 */
...
const sampleMakers = [
  {
    id: "m1",
    name: "Kijani Youth Co-op",
    region: "Morogoro",
    categories: ["Spices", "Food"],
    avatar: "🧺",
  },
  {
    id: "m2",
...
const sampleEvents = [
  {
    id: "e1",
    title: "SWAHIBA Pop-up Market + Live Performances",
    date: "Sat, 10 Feb",
    location: "Mwenge, Dar es Salaam",
  },
  {
    id: "e2",
    title: "Meet the Makers: Story Night",
    date: "Fri, 23 Feb",
    location: "Arusha CBD",
  },
  {
    id: "e3",
    title: "Youth Products Showcase",
    date: "Sun, 03 Mar",
    location: "Dodoma",
  },
];
...
export default function SwahibaMVP() {
  ...
  useScrollLock(filtersOpen || cartOpen || !!quickView || storyOpen);
  ...
  const addToCart = useCallback((product) => {
    setCart((c) => {
      const next = { ...c };
      if (next[product.id]) {
        next[product.id] = { product, qty: next[product.id].qty + 1 };
      } else {
        next[product.id] = { product, qty: 1 };
      }
      return next;
    });
    setCartOpen(true);
    ...
    <div className="min-h-screen bg-gradient-to-b from-zinc-50 via-white to-zinc-50 text-zinc-900">
      ...
      <header className="sticky top-0 z-30 border-b bg-white/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center gap-3 px-4 py-3 md:px-6">
          ...
          <nav className="hidden items-center gap-4 text-sm font-medium text-zinc-700 md:flex">
            ...
            <button type="button" onClick={() => setStoryOpen(true)} className="hover:text-black">
              SWAHIBA wa Siku
            </button>
      ...
      <main className="mx-auto max-w-6xl px-4 pb-24 pt-10 md:px-6">
        ...
        <section id="events" className="mt-16">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Pop-up markets, performances, and meet-the-maker moments.
          </p>
        ...
        <section id="makers" className="mt-16">
          ...
          <p className="mt-1 text-sm text-zinc-600">
            Real groups, real stories—shop their drops.
          </p>
